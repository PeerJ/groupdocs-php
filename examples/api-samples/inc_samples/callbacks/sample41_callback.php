...
$collaboratorGuid = $serializedData['collaboratorGuid'];
if ($documentGuid != "" && $collaboratorGuid != '') {
    ...
    if ($getCollaborators->status == "Ok") {
        ...
        for ($n = 0; $n < count($getCollaborators->result->collaborators); $n++) {
            ...
            $getCollaborators->result->collaborators[$n]->access_rights = 'CanView';
            //Add riviewer to ReviewerInfo array
            $reviewer[$n] = $getCollaborators->result->collaborators[$n];
