...
function share($userId, $privateKey, $body) {
    ...
    if ($userId == "" || $privateKey == "" || $body == "") {
        ...
        if ($url != "") {
            ...
            $uploadResult = $storageApi->UploadWeb($userId, $url);
        ...
        if ($_FILES['file']["name"] != "") {
            ...
            $uploadResult = $storageApi->Upload($userId, $name, 'uploaded', "", $fs);
        ...
        if ($fileId != "") {
            ...
            $files = $storageApi->ListEntities($userId, '', 0);
            if ($files->status == "Ok") {
        ...
        $share = $docApi->ShareDocument($userId, $file_id, $body);
        if ($share->status == "Ok") {
