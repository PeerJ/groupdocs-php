...
if (empty($clientId) || empty($privateKey) || empty($email) || empty($firstName) || empty($lastName)) {
    ...
    if ($basePath == "https://api.groupdocs.com/v2.0") {
        ...
        $url = 'http://realtime-apps.groupdocs.com/document-annotation2/embed/' . $fileId;
    }
        ...
        if ($allUsers->status == "Ok") {
            ...
            if ($getCollaborators->status == "Ok") {
                ...
                //If user wasn't founded in collaborators list - add him to it
                    ...
                    $setCollaberator = $antApi->SetAnnotationCollaborators($clientId, $fileId, "v2.0", $arrayEmail);
                    if ($setCollaberator->status == "Ok") {
                        // Check the result of the request
                        if (isset($setCollaberator->result)) {
                            //Add user GUID as "uid" parameter to the iframe URL
                            $url = $url . "?uid=" . $userGuid;
                            //Sign iframe URL
                            $url = $signer->signUrl($url);
                            // If request was successfull - set variables for template
                            F3::set('result', $setCollaberator->result);
                            F3::set("url", $url);
                        }
                    } else {
                        throw new Exception($setCollaberator->error_message);
                    }
                }
            } else {
